...
export function BadgesModal({ isOpen, onClose }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[200] flex items-end justify-center bg-black/85 p-0 backdrop-blur-md sm:items-center sm:p-4" onMouseDown={onClose}>
      <div className="flex h-[94dvh] w-full max-w-5xl flex-col overflow-hidden rounded-t-3xl border border-border-color bg-bg-secondary shadow-2xl sm:h-[92vh] sm:max-h-[820px] sm:rounded-3xl" onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="badges-modal-title">
        <header className="shrink-0 border-b border-border-color bg-[radial-gradient(circle_at_80%_0%,rgba(245,158,11,0.20),transparent_38%),linear-gradient(120deg,rgba(99,102,241,0.12),transparent)] px-5 py-5 sm:px-7">
          <div className="flex items-start justify-between gap-4">
            ...
            <button type="button" onClick={onClose} aria-label="Fechar conquistas" className="shrink-0 rounded-xl p-2 text-text-secondary transition-colors hover:bg-bg-primary/50 hover:text-text-primary"><X className="h-5 w-5" /></button>
          </div>
        </header>

        <section className="shrink-0 border-b border-border-color bg-bg-primary/20 px-5 py-4 sm:px-7" aria-labelledby="featured-badges-title">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 md:grid md:grid-cols-3 md:overflow-visible">
            {[0, 1, 2].map((slot) => {
              ...
              return badge ? (
                <button key={slot} type="button" onClick={() => toggleBadge(badge.id)} aria-label={`Remover ${badge.name} do destaque ${slot + 1}`} className={clsx('group flex min-h-16 w-[220px] shrink-0 items-center gap-2.5 rounded-2xl border-2 p-2.5 text-left transition-colors md:w-auto', badge.bg, badge.border, 'hover:border-red-400/60')}>
                ...
                <div key={slot} className="flex min-h-16 w-[220px] shrink-0 items-center gap-3 rounded-2xl border border-dashed border-border-color bg-bg-primary/30 p-3 md:w-auto">
        ...
        <div className="flex shrink-0 items-center gap-2 overflow-x-auto border-b border-border-color px-5 py-3 sm:px-7" aria-label="Filtros de conquistas">
          {FILTERS.map((item) => {
            const count = item.id === 'all' ? BADGES.length : item.id === 'unlocked' ? unlockedBadges.length : BADGES.length - unlockedBadges.length;
            return (
              <button key={item.id} type="button" onClick={() => setFilter(item.id)} aria-pressed={filter === item.id} className={clsx('shrink-0 rounded-xl px-3 py-2 text-xs font-black transition-colors', filter === item.id ? 'bg-button-accent text-text-on-primary shadow-md shadow-button-accent/15' : 'bg-bg-tertiary text-text-secondary hover:text-text-primary')}>
            ...
            {visibleBadges.map((badge) => {
              ...
              return (
                <button
                  ...
                  className={clsx(
                    'flex h-full min-h-52 flex-col rounded-2xl border p-4 text-left transition-all',
        ...
        <footer className="flex shrink-0 flex-col gap-3 border-t border-border-color bg-bg-tertiary/35 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7">
          ...
          <div className="grid grid-cols-2 gap-2 sm:flex">
            <button type="button" onClick={onClose} className="rounded-xl border border-border-color px-4 py-2.5 text-sm font-bold text-text-secondary transition-colors hover:text-text-primary">Cancelar</button>
            <button type="button" onClick={handleSave} disabled={saving} className="inline-flex min-w-32 items-center justify-center gap-2 rounded-xl bg-button-accent px-5 py-2.5 text-sm font-black text-text-on-primary shadow-md shadow-button-accent/15 disabled:opacity-60"><Save className="h-4 w-4" />{saving ? 'Salvando...' : 'Salvar vitrine'}</button>
          </div>
        </footer>
      </div>
    </div>,
