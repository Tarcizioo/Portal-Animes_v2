...
export function AchievementBadges({ readOnly = false, publicLibrary = null, publicProfile = null }) {
  ...
  return (
    <section aria-labelledby="profile-achievements-title" className="min-w-0 max-w-full overflow-hidden rounded-2xl border border-border-color bg-bg-secondary shadow-lg shadow-black/5">
        ...
        <div className="relative flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <h3 id="profile-achievements-title" className="text-lg font-black text-text-primary">Conquistas</h3>
              <p className="text-xs text-text-secondary">{unlockedBadges.length} de {BADGES.length} desbloqueadas</p>
            </div>
          </div>
          {!readOnly && (
            <button type="button" onClick={() => setIsModalOpen(true)} aria-label="Gerenciar conquistas" className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-border-color bg-bg-primary/40 text-text-secondary transition-colors hover:border-button-accent/50 hover:text-text-primary"><Settings2 className="h-4 w-4" aria-hidden="true" /></button>
          )}
        </div>
      ...
      <div className="min-w-0 p-5 md:p-6">
        ...
        {featuredBadges.length > 0 ? (
          <div data-testid="featured-badges-grid" className="grid min-w-0 max-w-full grid-cols-[minmax(0,1fr)] gap-2 sm:grid-cols-[repeat(3,minmax(0,1fr))]">
            {featuredBadges.map((badge) => {
              const Icon = badge.icon;
              return (
                <div key={badge.id} className={clsx('group flex w-full min-w-0 max-w-full items-center gap-3 rounded-2xl border p-3 transition-transform hover:-translate-y-0.5', badge.bg, badge.border)}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 truncate text-sm font-black text-text-primary" title={badge.name}>{badge.name}</p>
                    <p className="mt-0.5 truncate text-[10px] text-text-secondary" title={badge.description}>{badge.description}</p>
                  </div>
                </div>
              );
            })}
        ...
        {nextBadge && nextProgress && (
          <button type="button" onClick={() => !readOnly && setIsModalOpen(true)} disabled={readOnly} className="mt-4 flex min-h-14 w-full min-w-0 max-w-full items-center gap-3 rounded-2xl border border-border-color bg-bg-tertiary/35 p-3 text-left transition-colors enabled:hover:border-button-accent/40">
